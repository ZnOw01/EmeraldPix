...
export const BLANK_TILE_RETRY_DELAY_MS = 600;
export const BLANK_TILE_MAX_RETRIES = 3;
...
/**
 * Check if a URL can be captured by extension policy
 */
export function isCapturableUrl(url?: string): boolean {
	if (!url) return false;
	const normalized = url.toLowerCase();
	if (!CAPTURE_PROTOCOLS.some((p) => normalized.startsWith(p))) return false;
	if (BLOCKED_URLS.some((p) => p.test(normalized))) return false;
	if (BLOCKED_HTTP_URLS.some((p) => p.test(normalized))) return false;
	return true;
}
