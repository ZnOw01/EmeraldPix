...
function wrapContentScriptIifePlugin(): PluginOption {
	return {
		name: "emeraldpix-wrap-content-script-iife",
		generateBundle(_options, bundle) {
			const chunk = bundle["assets/content_script.js"];
			if (chunk && "code" in chunk) {
				// Wrap in IIFE so `const`/`let` declarations are scoped to each injection.
				// This prevents "Identifier has already been declared" when Chrome
				// injects the content script multiple times into the same JS realm.
				chunk.code = `(function(){"use strict";${chunk.code}})();`;
			}
		},
	};
}
